...
public class LowbarLowGoal implements AutonomousController {
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				if(AutoChooseUtil.getInstance().getFireSettings() != AutoChooseUtil.AutoFireDecision.DontFire) {
					...
					// Run
					if(running) DriveTrainUtil.driveDistanceSequential(151, 0.4);
					if(running) SubSystemsManager.getInstance().getShooter().fire();
					// Turn around
